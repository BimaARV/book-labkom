    ...
    public function boot(): void
    {
        try {
            if (\Illuminate\Support\Facades\Schema::hasTable('settings')) {
                ...
                if ($appUrl) {
                    // Force https for the URL used in emails and notifications
                    $appUrl = str_replace('http://', 'https://', $appUrl);
                    
                    config(['app.url' => $appUrl]);
                    
                    // Allow the app to dynamically resolve the host from the request for assets
                    // We don't forceRootUrl globally so it won't break if accessed via localhost or IP
                    // Force HTTPS if the URL uses https
                    if (str_starts_with($appUrl, 'https://')) {
                        \Illuminate\Support\Facades\URL::forceScheme('https');
                    }

                    // Fix CLI URLs generating localhost (for queue workers sending emails/WA)
